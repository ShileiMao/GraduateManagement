<template>
  <div class="login">
    <div class="form-box" v-loading="loading" element-loading-background="rgba(0, 0, 0, 0)">
      <div class="title"><h1 title="Lin">论文评分系统</h1></div>
  ...
  methods: {
    async login() {
      ...
      try {
        ...
        console.log('going to get user information')
    ...
    async getInformation() {
      try {
        console.log("starting get information")
        // 尝试获取当前用户信息
        const user = await User.getInformation()
        const permission = await User.getPermissions()
        user.admin = permission.admin

        console.log("login vue, userInformation: " + JSON.stringify(user))

        console.log("login vue, permission " + JSON.stringify(permission))
        this.setUserAndState(user)
        this.setUserPermissions(permission.permissions)
      } catch (e) {
        console.log(e)
      }
    },
