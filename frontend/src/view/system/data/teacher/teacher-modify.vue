<template>
  <div class="container">
    <div class="title">
      <span>修改教师</span> <span class="back" @click="back"> <i class="iconfont icon-fanhui"></i> 返回 </span>
    </div>
        ...
        <el-col :lg="16" :md="20" :sm="24" :xs="24">
          <el-form :model="form" status-icon ref="form" label-width="100px" v-loading="loading" @submit.native.prevent>
            ...
            <el-form-item label="职称" prop="position">
              <el-input size="medium" v-model="form.position" placeholder="请填写职称"></el-input>
            </el-form-item>
...
export default {
  ...
  async mounted() {
    this.loading = true
    this.form = await teacher.getTeacher(this.editTeacherID)
    this.form.sex = this.form.sex != null ? this.form.sex.toString() : null
    this.loading = false
  },
  methods: {
    async submitForm() {
      const res = await teacher.editTeacher(this.editTeacherID, this.form)
